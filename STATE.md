...
| | |
| --- | --- |
| **Heartbeat** | 2026-08-10 (S6 **send half** — `OutboundQueue`, the p2e send path's six ordering decisions moved into `:core`, plus the 409 `latest` the relay was already sending and `RelayClient` was discarding; cloud iteration, Linux sandbox, **tenth** run). `:core` only: **two new files, four edits confined to one type change, zero `:app` files, zero main-repo files.** `:core` 154 → **177 / 0 / 0**, measured here. **S6 stays PARTIAL — the label was right, its stated reason was not** |
| **Previous heartbeat** | 2026-08-10 (S3 decision half — `PairingFlow` + `RelayTokenLadder`; **ninth** run). `:core` 133 → 154 / 0 / 0. **S3 BLOCKED → PARTIAL** |
