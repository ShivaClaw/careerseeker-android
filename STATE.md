...
| **CI on this push** | **NOT YET OBSERVED from this session.** The fourteenth-run push is `claude/android-a0-probe` (`:core` only) and `claude/s4-pull-request-semantics` (one doc). Neither gate ran here and neither can — no Android SDK/JBR, `dl.google.com` egress-denied (B-7, re-measured: `CONNECT tunnel failed, response 403`); no .NET for `Verify-Alpha.ps1`. **Poll with the MCP `get_check_runs` method — the Actions REST API is 403 to `curl` here** (C-S6A-1 said so and the twelfth run hit it anyway). **190** is the probe's figure and is not gate-corroborated as a count |
