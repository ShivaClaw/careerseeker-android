...
| **CI on this push** | **NOT YET REPORTED at time of writing** (tenth run) — the S6-send commits were pushed at the end of this iteration and no run had concluded when these records were written. **Do not read the row below as covering them.** When it reports, check `head_sha` against this branch's tip and count `() PASSED` lines: expect **177** (method in C-S3A-9). |
