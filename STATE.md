...
| **Main-repo PR open** | **#32 draft** — `claude/s5-entitlement-ack-spec`, S5 spec + vectors + the relay size-cap fix (head `9c05ef7`, CI green). **Not merged** (merging needs a full local gate this machine cannot run) · **#33 draft** — `claude/s4-pull-request-semantics`, **stacked on #32**, S4's spec half — §4.3.4 plus, as of 2026-08-11, **§2.1's pull response body** (head `10696d2`, **CI not yet observed on this head from this session**) · **NEW: `claude/s2-relay-retention`**, also **stacked on #32** (head `310406a`), the retention fix + 6 tests. Whether #33 and the new branch should be retargeted at `main` once #32 lands is on the return-day list |
...
| **Coordination bus** | `autonomy/claude-state` — updated this iteration (thirteenth run); files claimed named there. **This iteration wrote one main-repo file**, `docs/Sync-Protocol.md` on `claude/s4-pull-request-semantics` — already claimed territory via #32/#33, so no new claim was taken. Terra read at iteration start and again before writing: R6(b) BLOCKED, heartbeat unchanged, **claims no files** — no collision |
...
thirteenth run). S2's `/pair` page and S5's C# applier need .NET; S4's `:app` wiring, S6's controls
and every screen need an Android SDK (B-7); S3's key and any hardware claim need B-4. **What is left
that a sandbox can still reach:**

1. **PQ-S4-3** — how far may an unparseable element move a receiver's cursor? Opened by this slice
   and **the strongest remaining candidate**: the decision is a spec sentence (§2.1 or §6.2) and the
   implementation half is `:core`, so both ends are reachable here. Spec first, as always. It is a
   *decision* — refusing to advance stalls the direction forever, which §6.2 forbids; advancing by
   one desynchronises the cursor from real sequence numbers; bounding by the page's own `latest`
   looks best and needs writing down before it is coded.
2. `relay/` (Node + vitest + miniflare, no egress denial). Re-read once since the size-cap fix — the
   eleventh run's retention slice — so it is no longer untouched territory, but PQ-S2-1 and PQ-S2-2
   are still open there and still say **do not close either from a sandbox** (spec first, relay
   second, on a machine with a gate).
3. Whatever `:core` decision layers remain unenumerated.
