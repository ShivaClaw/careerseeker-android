...
> - It travels through a relay server we run. The relay stores **only encrypted blobs**
>   — it cannot read them, and neither can we. It sees a random pairing ID, message
>   sizes, and timestamps. It never sees your name, email, documents, or job list.
...
| Data shared? | No | No third parties, no SDKs |
...
**Open question for the gate:** whether "we run the relay but cannot read it" should name
the operator ("a server we operate") as above, or name Cloudflare as the infrastructure
provider explicitly. Recommendation: name Cloudflare in the privacy policy (it is true and
auditable), keep the in-app copy short as written.
