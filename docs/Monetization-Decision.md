...
| Product | Price | Notes |
| --- | --- | --- |
| **CareerSeeker Basic** (.exe) | **Free forever** | Windows engine. No locked features. |
...
## 3. Naming note (worth a decision, not urgent)

"CareerSeeker **Basic**" is new — the site and spec call the Windows app simply
"CareerSeeker". "Basic" names a *tier*, which implies a ladder with things withheld at the
bottom, and that pulls against the strongest promise on the page ("no locked features").

Suggestion: keep the Windows app **"CareerSeeker"** — the product — with Dashboard, Pro,
and Cloud as named add-ons. It costs nothing to keep, and avoids a reviewer or user reading
"Basic" as "crippled". If "Basic" is intended for internal SKU clarity only, that works
fine without appearing in user-facing copy.
