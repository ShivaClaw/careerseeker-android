...
note ""
note "BEFORE YOU APPLY THAT LESSON TO B-7, READ THIS — run 242 already did, and the"
note "obvious probe gives the WRONG ANSWER (C-242-2). B-7 is the ONE 'this sandbox"
note "cannot' on the board that IS measured and DOES hold. Re-measure it with the"
note "instrument, never with a hand-written curl:"
note ""
note "    scripts/b7-probe.sh      # exit 0 = B-7 HOLDS; exit 1 = it may have lifted"
note ""
note "  Two things that will mislead you if you roll your own:"
note "    1. maven.google.com answers HTTP 301 AT ITS ROOT. That is NOT reachability —"
note "       the redirect lands on dl.google.com, which the policy denies, so every"
note "       ARTIFACT fetch still dies 403. Probe an artifact PATH, never a host root."
note "    2. dsl.maven.google.com — the host C-ENV-1 measured for 196 firings — HAS NO"
note "       DNS RECORD. Its 000 meant 'no such name', not 'denied', so that command"
note "       could never have detected a change. That is C-242-1, and B-7's CONCLUSION"
note "       was never affected: BLOCKED.md named the real host and the real 403 on"
note "       day one."
note ""
note "  Last VERIFIED (run 242, 2026-09-17, first-person on this image): the pinned AGP"
note "  pom returns CONNECT 403 / HTTP 000 on BOTH dl.google.com and maven.google.com,"
note "  control repo1.maven.org reached. If you re-measure, MOVE THIS STAMP."
